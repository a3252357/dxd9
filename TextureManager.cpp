...
shared_ptr<Texture2d> TextureManager::getTexture(LPCWSTR path, int x, int y, int w, int h)
{
	...
	for (iter = TextureMap.rbegin(); iter != TextureMap.rend(); iter++)
	{
		if (lstrcmpW(iter->first, path) == 0)
			return make_shared<Texture2d>(&iter->second, x, y, w, h);
	}
	...
	if (hr == S_OK) {
		...
		return make_shared<Texture2d>(&ptexture9, x, y, w, h);
	}
	else {
		return NULL;
	}
